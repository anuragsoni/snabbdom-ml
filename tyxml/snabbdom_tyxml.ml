...
(* overriding the [svg] function generated by the tyxml functor since it adds some
   namespace attribures. We don't need those when creating svg nodes using snabbdom. *)
let svg' ?a children = Vnode.make_node "svg" (Xml_vnode.make_attrs a) children

module Svg = struct
  include Svg_f.Make (Svg_vnode)

  let svg = svg'
end

module Html = struct
  include Html_f.Make (Xml_vnode) (Svg)
